...
- **Structure is flat and hand-rolled.** No CV class or bibliography tooling — no `.bib`, no BibTeX/biber. **Every item in every section is one `\entry{...}`**, a custom macro (defined in the preamble) that hanging-indents by 1.5em. There is no second entry shape and no `\hfill`. What varies is the spacing, declared once per section:
  - **Prose sections** (Publications, Works in Progress, Presentations) use bare `\entry` lines, which get 3pt of `\parskip` between them, because these entries usually run to two or more lines and need the separation.
  - **One-line sections** (Appointments, Education, Awards, Teaching) wrap their entries in `\begin{tight}...\end{tight}`, which zeroes `\parskip` locally. Without it the same ~3pt per entry adds ~45pt overall and spills Teaching onto a third page.
- **One-line entries read `{thing}, {institution}, {years}.`** — comma-separated, closing period. `\hfill` right-aligned year columns were removed; don't reintroduce them.
...
- **Citations follow a fixed shape**: `Author(s), ``Title,'' \textit{Venue} vol.issue: pages, year. \doi{10.xxxx/yyyy}`. Use TeX quotes (` `` ` and `''`), `--` for year and page ranges, and prefer a DOI over a publisher link. `\doi{...}` takes the bare DOI and supplies the `https://doi.org/` prefix; non-DOI links (the dissertation PURL, the CMLC paper) use `\url{}` directly. Items not yet assigned a DOI simply omit the link.
